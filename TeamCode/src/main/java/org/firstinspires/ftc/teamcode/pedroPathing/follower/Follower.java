...
public class Follower {
    ...
    public static boolean useTeleHeading = true;

    private double lastHeading;
    private boolean changingHeading;
    private boolean changingHeadingLast = false;
    ...
    public Action follow(Path a) {
        //return new InstantAction(() -> followPath(a));
        return new FollowAction(a);
    }

    public Action instantFollow(Path a) {
        return new InstantAction(() -> followPath(a));
    }
    ...
    public void setTeleOpMovementVectors(double forwardDrive, double lateralDrive, double heading) {
        if(!teleopDrive && Math.abs(forwardDrive + lateralDrive + heading)>0) {
            startTeleopDrive();
        }
    ...
    public void setTeleOpMovementVectors(double forwardDrive, double lateralDrive, double heading, boolean robotCentric) {
        changingHeading = Math.abs(heading)>0.00000001;
        
        if(changingHeading) {
            lastHeading = poseUpdater.getTotalHeading();
        }
        ...
        if(!changingHeading && useTeleHeading) {
            headingError = MathFunctions.getTurnDirection(poseUpdater.getPose().getHeading(), lastHeading) * MathFunctions.getSmallestAngleDifference(poseUpdater.getPose().getHeading(), lastHeading);
            headingPIDF.updateError(headingError);
            teleopHeadingVector = new Vector(MathFunctions.clamp(headingPIDF.runPIDF() + headingPIDFFeedForward * MathFunctions.getTurnDirection(poseUpdater.getPose().getHeading(), lastHeading), -1, 1), poseUpdater.getPose().getHeading());
            return;
        }
